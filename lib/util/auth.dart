...
class Auth {
  static Future<String> signUp(String email, String password) async {
    ...
    static Future<String> parentsignUp(String parentemail, String parentpassword) async {
    FirebaseUser parentuser = (await FirebaseAuth.instance
        .createUserWithEmailAndPassword(email: parentemail, password: parentpassword)).user;
    return parentuser.uid;
  }

  static void addUserSettingsDB(User user) async {
    checkUserExist(user.userId).then((value) {
      if (!value) {
        print("dealer ${user.firstName} ${user.email} added");
        Firestore.instance
            .document("dealers/${user.userId}")
            .setData(user.toJson());
        _addSettings(new Settings(
          settingsId: user.userId,
        ));
      } else {
        print("dealer ${user.firstName} ${user.email} exists");
      }
    });
  }

    static void addParentUserSettingsDB(ParentUser parentuser) async {
    checkUserExist(parentuser.parentId).then((value) {
      if (!value) {
        print("user ${parentuser.parentName} ${parentuser.parentemail} added");
        Firestore.instance
            .document("users/${parentuser.parentId}")
            .setData(parentuser.toJson());
        _addSettings(new Settings(
          settingsId: parentuser.parentId,
        ));
      } else {
        print("user ${parentuser.parentName} ${parentuser.parentemail} exists");
      }
    });
  }
  ...
  static Future<bool> checkUserExist(String userId) async {
    bool exists = false;
    try {
      await Firestore.instance.document("dealers/$userId").get().then((doc) {
        if (doc.exists)
          exists = true;
        else
          exists = false;
    ...
    static Future<bool> checkParentUserExist(String parentId) async {
    bool exists = false;
    try {
      await Firestore.instance.document("users/$parentId").get().then((doc) {
        if (doc.exists)
          exists = true;
        else
          exists = false;
  ...
  static Future<String> signIn(String email, String password) async {
    ...
    static Future<String> parentsignIn(String parentemail, String parentpassword) async {
    FirebaseUser parentuser = (await FirebaseAuth.instance
        .signInWithEmailAndPassword(email: parentemail, password: parentpassword)).user;
    return parentuser.uid;
  }
  ...
  static Future<User> getUserFirestore(String userId) async {
    ...
    static Future<ParentUser> getParentUserFirestore(String parentId) async {
    if (parentId != null) {
      return Firestore.instance
          .collection('users')
          .document(parentId)
          .get()
          .then((documentSnapshot) => ParentUser.fromDocument(documentSnapshot));
    } else {
      print('firestore userId can not be null');
      return null;
    }
  ...
  static Future<String> storeUserLocal(User user) async {
    ...
    await prefs.setString('dealers', storeUser);
    return user.userId;
  }

    static Future<String> storeParentUserLocal(ParentUser parentuser) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String storeParentUser = parentuserToJson(parentuser);
    await prefs.setString('users', storeParentUser);
  ...
  static Future<User> getUserLocal() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (prefs.getString('dealers') != null) {
      User user = userFromJson(prefs.getString('dealers'));
      //print('USER: $user');
      return user;
    } else {
      return null;
    ...
    static Future<ParentUser> getParentUserLocal() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (prefs.getString('users') != null) {
      ParentUser parentuser = parentuserFromJson(prefs.getString('users'));
      //print('USER: $user');
      return parentuser;
    } else {
      return null;
  ...
  static Future<void> forgotPasswordEmail(String email) async {
    ...
    static Future<void> forgotParentPasswordEmail(String parentemail) async {
