...
class _StateWidgetState extends State<StateWidget> {
  ...
  Future<Null> initUser() async {
    ...
    Future<Null> initParentUser() async {
    //print('...initUser...');
    FirebaseUser firebaseParentUserAuth = await Auth.getCurrentFirebaseUser();
    ParentUser parentuser = await Auth.getParentUserLocal();
    Settings settings = await Auth.getSettingsLocal();
    setState(() {
      state.isLoading = false;
      state.firebaseUserAuth = firebaseParentUserAuth;
  ...
  Future<void> logInUser(email, password) async {
    ...
    Future<void> logInParentUser(parentemail, parentpassword) async {
    String parentId = await Auth.parentsignIn(parentemail, parentpassword);
    ParentUser parentuser = await Auth.getParentUserFirestore(parentId);
    await Auth.storeParentUserLocal(parentuser);
    Settings settings = await Auth.getSettingsFirestore(parentId);
    await Auth.storeSettingsLocal(settings);
    await initParentUser();
  }
